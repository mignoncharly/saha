...
> **Purpose of this file.** Contributors and coding tools should read this guide
> before making changes. It documents the project architecture, production
> constraints, validation commands, and safe development workflow. Keep it
> current whenever the architecture or deployment process changes.

---

## âš ï¸ Read this first â€” production safety
...
- To validate changes, prefer: type-checking, linting, reading code, and unit
  tests in an isolated environment â€” not by starting servers here.
...
**SAHA Transport & Logistics (STL)** â€” a web app (MVP) for a transport &
...
- Two deployments behind Nginx:
  - Frontend â†’ `https://saha-stl.docufisc.de`
  - API â†’ `https://api-saha.docufisc.de`

---

## Tech stack

### Backend â€” `backend/` (Django + DRF)
...
**Django apps (`backend/apps/`)** â€” each is a domain:
| App | Responsibility |
|-----|----------------|
...
| `logistics` | transport requests â€” core domain (`/api/transport-requests/`) |
...
### Frontend â€” `frontend/` (Next.js 14 App Router + TypeScript + Tailwind)
...
- `app/` â€” App Router pages. Public routes retain their French URL slugs but
...
| `saha-worker.service` | Celery worker | â€” |
| `saha-beat.service` | Celery beat (scheduler) | â€” |
| `saha-redis.service` | Redis | â€” |
...
- **Restart only when a change requires it and you understand the impact.**
  - Backend code change â†’ restart `saha-api` (and `saha-worker`/`saha-beat` if tasks changed).
  - Frontend change â†’ `npm run build` in `frontend/`, then restart `saha-frontend`.
...
- **Frontend:** `cd frontend && npm run lint`. Type-check via `tsc`/`next build`
  ...
  shows as modified â€” usually safe to ignore in diffs).
...
4. Verify with targeted tests / lint / type-check in an isolated environment â€”
