...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(setRequests)
      .catch(() => setError(t("Impossible de charger vos demandes.")))
  ...
  const resendVerification = async () => {
    ...
    try {
      ...
      toast.success(t("Email de vérification renvoyé."));
    } catch {
      toast.error(t("Impossible de renvoyer l'email pour le moment."));
    } finally {
      setResending(false);
    }
  };
  ...
  const quickActions = [
    { href: "/demande", label: t("cta.request"), icon: Truck, accent: "bg-brand-red/10 text-brand-red" },
    { href: "/suivi", label: t("Mes demandes"), icon: Package, accent: "bg-brand-blue/10 text-brand-blue" },
    { href: "/calendrier", label: t("Calendrier"), icon: CalendarDays, accent: "bg-brand-gold/10 text-brand-gold" },
  ];
    ...
    <>
      <section className="bg-gradient-to-br from-brand-blue to-navy-900 text-white">
        <div className="container-page flex flex-col gap-4 py-10 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-blue-100">{t("Bienvenue,")}</p>
            <h1 className="font-display text-3xl font-bold capitalize">{userDisplayName(user, t("account.mySpace"))}</h1>
            <p className="mt-1 text-sm text-blue-100">{user.email}</p>
          </div>
          <div className="flex gap-3">
            <Link href="/demande" className="btn-primary !px-5 !py-2.5">
              <Plus className="h-4 w-4" /> {t("Nouvelle demande")}
            </Link>
      ...
      <div className="container-page space-y-10 py-10">
        ...
        {user.email_verified === false && (
          <div className="flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-amber-900">{t("Vérifiez votre adresse email")}</p>
                <p className="text-sm text-amber-800">{t("Confirmez votre email pour sécuriser votre compte.")}</p>
              </div>
            </div>
        ...
        {/* Quick actions */}
        <section>
          <h2 className="mb-4 text-lg font-bold text-gray-900">{t("Actions rapides")}</h2>
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-bold text-gray-900">{t("Mes demandes récentes")}</h2>
            {requests.length > 0 && (
        ...
        {/* Notifications */}
        <section>
          <h2 className="mb-4 text-lg font-bold text-gray-900">{t("Notifications")}</h2>
          <div className="flex flex-col gap-4 rounded-2xl border border-gray-100 bg-white p-6 shadow-card sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-900">{t("Alertes de ramassage")}</p>
                <p className="text-sm text-gray-600">
                  {t("Activez les notifications push et gérez vos préférences et votre historique.")}
                </p>
              </div>
            </div>
