...
function ConfirmationInner() {
  ...
  const copyRef = async () => {
    try {
      ...
      setCopied(true);
      toast.success(t("Référence copiée"));
      setTimeout(() => setCopied(false), 2000);
    } catch {
      toast.error(t("Impossible de copier la référence"));
    }
  };
    ...
    <div className="container-page max-w-xl py-16">
      ...
      {ref && (
        <div className="mt-8 rounded-2xl border border-gray-100 bg-white p-6 text-center shadow-card">
          <p className="text-sm text-gray-500">{t("Votre numéro de référence")}</p>
          ...
          <p className="mt-1 text-xs text-gray-400">{t("Conservez ce numéro pour suivre votre demande.")}</p>
          ...
          {trackingUrl && (
            <div className="mt-5 flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-gray-400">{t("Scannez pour suivre votre demande")}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="mt-8 flex flex-col items-stretch gap-3">
        ...
        {ref && (
          <Link href={`/suivi?ref=${ref}`} className="btn-secondary w-full">
            {t("Voir le suivi")} <ArrowRight className="h-4 w-4" />
          </Link>
        )}
        <Link href="/" className="btn-ghost w-full">
          <Home className="h-4 w-4" /> {t("Retour à l'accueil")}
        </Link>
      </div>
      ...
      {!user && (
        <div className="mt-8 rounded-2xl border border-brand-blue/15 bg-brand-blue/5 p-5 text-center text-sm text-gray-700">
          <p>
            {t("Créez un compte pour retrouver toutes vos demandes et recevoir des notifications de suivi.")}
          </p>
