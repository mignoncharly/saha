...
export default function AdminNotificationComposer() {
  ...
  const handleSend = async () => {
    if (!title || !body) return toast.error(t("Titre et message sont requis."));
    setSending(true);
    try {
  ...
  return (
    <div className="max-w-xl space-y-5">
      <h2 className="text-lg font-bold text-gray-900">{t("Envoyer une notification push")}</h2>

      <div className="card space-y-4">
        <FormField label={t("Titre")} htmlFor="notif-title" required>
        ...
        <FormField label={t("Cible")} htmlFor="notif-target">
          <select id="notif-target" value={targetType} onChange={(e) => setTargetType(e.target.value)} className="input">
            <option value="all">{t("Tous les abonnés")}</option>
            <option value="region">{t("Par région")}</option>
          </select>
        </FormField>
        {targetType === "region" && (
          <FormField label={t("Région")} htmlFor="notif-region">
            <input id="notif-region" value={targetRegion} onChange={(e) => setTargetRegion(e.target.value)} placeholder={t("ex: Frankfurt")} className="input" />
          </FormField>
        )}
        <button onClick={handleSend} disabled={sending} className="btn-primary w-full">
          {sending ? <LoadingSpinner className="h-5 w-5" /> : <><Send className="h-4 w-4" /> {t("Envoyer la notification")}</>}
        </button>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-brand-blue/15 bg-brand-blue/5 p-4 text-sm text-gray-600">
        ...
        <p>{t("La notification est envoyée aux utilisateurs ayant activé les notifications push sur leur appareil.")}</p>
      </div>
    </div>
  );
}
